...
const QRCodeIcon = () => `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h2M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" /></svg>`;
const DownloadIcon = () => `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>`;
...
let state = {
    ...
    // App Mode
    appMode: 'transcript', // 'transcript', 'live', 'records', 'statistics'
    ...
    // Current record ID (for QR code generation)
...
const statisticsModeBtn = document.getElementById('statistics-mode-btn');
// QR Modal Elements
...
const renderOverviewView = (data) => {
    ...
    const qrButton = state.currentRecordId 
        ? `<button id="qr-code-btn" class="bg-purple-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-colors duration-200 flex items-center ml-2">${QRCodeIcon()} Email via QR</button>`
        : '';
    
    const downloadButton = state.currentRecordId
        ? `<button id="download-pdf-btn" class="bg-slate-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 transition-colors duration-200 flex items-center ml-2">${DownloadIcon()} Download PDF</button>`
        : '';

    const patientHeader = `<div class='bg-white p-6 rounded-xl shadow-md border border-slate-200'><div class="flex justify-between items-start flex-wrap gap-2"><div><h2 class="text-2xl font-bold text-slate-800">${data.patient_name}</h2><p class="text-slate-500">Patient ID: ${data.patient_id} · Age: ${data.age}</p></div><div class="flex flex-wrap">${saveButton}${qrButton}${downloadButton}</div></div></div>`;
    ...
    recordsList.innerHTML = sortedRecords.map(record => `
        <div class="bg-slate-50 p-4 rounded-lg shadow-sm border flex justify-between items-center hover:bg-slate-100 transition-colors duration-200">
            ...
            <div class="flex space-x-2">
                ...
                <button data-record-id="${record.id}" class="qr-record-btn text-sm bg-purple-100 text-purple-700 font-semibold py-2 px-4 rounded-md hover:bg-purple-200 transition-colors duration-200">📱 QR</button>
                <button data-record-id="${record.id}" class="pdf-record-btn text-sm bg-blue-100 text-blue-700 font-semibold py-2 px-4 rounded-md hover:bg-blue-200 transition-colors duration-200">📄 PDF</button>
...
const renderStatistics = () => {
    ...
    document.getElementById('stat-total-analyses').textContent = stats.allTime?.totalAnalyses || 0;
    document.getElementById('stat-avg-risk').textContent = Math.round(stats.allTime?.averageRiskScore || 0) + '%';
    document.getElementById('stat-this-week').textContent = stats.currentWeek?.totalAnalyses || 0;
    document.getElementById('stat-high-risk').textContent = (stats.allTime?.riskLevels?.high || 0) + (stats.allTime?.riskLevels?.critical || 0);

    // Render common diseases chart
    const diseasesChart = document.getElementById('common-diseases-chart');
    if (stats.allTime?.topDiseases?.length > 0) {
        const maxCount = Math.max(...stats.allTime.topDiseases.map(d => d.count));
        diseasesChart.innerHTML = stats.allTime.topDiseases.slice(0, 8).map(disease => `
            <div class="flex items-center">
                <span class="w-32 text-sm text-slate-600 truncate">${disease.name}</span>
                <div class="flex-1 mx-2">
                    <div class="bg-slate-200 rounded-full h-4 overflow-hidden">
                        <div class="bg-blue-500 h-full rounded-full transition-all duration-500" style="width: ${(disease.count / maxCount) * 100}%"></div>
                    </div>
                </div>
                <span class="w-8 text-sm font-semibold text-slate-700 text-right">${disease.count}</span>
            </div>
        `).join('');
    } else {
        diseasesChart.innerHTML = '<p class="text-slate-500 text-center py-8">No disease data available yet</p>';
    }

    // Render risk distribution chart
    const riskChart = document.getElementById('risk-distribution-chart');
    const riskLevels = stats.allTime?.riskLevels || { low: 0, medium: 0, high: 0, critical: 0 };
    const totalRisk = Object.values(riskLevels).reduce((a, b) => a + b, 0);
    ...
    if (totalRisk > 0) {
        ...
        riskChart.innerHTML = Object.entries(riskLevels).map(([level, count]) => `
            <div class="flex items-center">
                <span class="w-24 text-sm text-slate-600 flex items-center">
                    <span class="w-3 h-3 rounded-full mr-2" style="background-color: ${riskColors[level]}"></span>
                    ${riskLabels[level]}
                </span>
                <div class="flex-1 mx-2">
                    <div class="bg-slate-200 rounded-full h-4 overflow-hidden">
                        <div class="h-full rounded-full transition-all duration-500" style="background-color: ${riskColors[level]}; width: ${(count / totalRisk) * 100}%"></div>
                    </div>
                </div>
                <span class="w-12 text-sm font-semibold text-slate-700 text-right">${count} (${Math.round((count / totalRisk) * 100)}%)</span>
            </div>
        `).join('');
    } else {
        riskChart.innerHTML = '<p class="text-slate-500 text-center py-8">No risk data available yet</p>';
    }

    // Render top medications chart
    const medsChart = document.getElementById('top-medications-chart');
    if (stats.allTime?.topMedications?.length > 0) {
        const maxMedCount = Math.max(...stats.allTime.topMedications.map(m => m.count));
        medsChart.innerHTML = stats.allTime.topMedications.slice(0, 8).map(med => `
            <div class="flex items-center">
                <span class="w-32 text-sm text-slate-600 truncate">${med.name}</span>
                <div class="flex-1 mx-2">
                    <div class="bg-slate-200 rounded-full h-4 overflow-hidden">
                        <div class="bg-purple-500 h-full rounded-full transition-all duration-500" style="width: ${(med.count / maxMedCount) * 100}%"></div>
                    </div>
                </div>
                <span class="w-8 text-sm font-semibold text-slate-700 text-right">${med.count}</span>
            </div>
        `).join('');
    } else {
        medsChart.innerHTML = '<p class="text-slate-500 text-center py-8">No medication data available yet</p>';
    }

    // Render weekly trend
    const trendChart = document.getElementById('weekly-trend-chart');
    if (stats.recentWeeks?.length > 0) {
        const maxWeekly = Math.max(...stats.recentWeeks.map(w => w.totalAnalyses || 0), 1);
        trendChart.innerHTML = stats.recentWeeks.reverse().map(week => `
            <div class="flex items-center">
                <span class="w-20 text-sm text-slate-600">Week ${week.weekNumber}</span>
                <div class="flex-1 mx-2">
                    <div class="bg-slate-200 rounded-full h-4 overflow-hidden">
                        <div class="bg-green-500 h-full rounded-full transition-all duration-500" style="width: ${(week.totalAnalyses / maxWeekly) * 100}%"></div>
                    </div>
                </div>
                <span class="w-16 text-sm font-semibold text-slate-700 text-right">${week.totalAnalyses} cases</span>
            </div>
        `).join('');
    } else {
        trendChart.innerHTML = '<p class="text-slate-500 text-center py-8">No weekly data available yet</p>';
    }
};
...
const render = () => {
    ...
    statisticsModeBtn.classList.remove('active');
    ...
    } else if (state.appMode === 'statistics') {
        statisticsView.classList.remove('hidden');
        statisticsModeBtn.classList.add('active');
        renderStatistics();
    }
...
const saveCurrentRecord = async () => {
    ...
    try {
        ...
        state.currentRecordId = savedRecord._id; // Store the record ID for QR code generation
...
// --- QR CODE FUNCTIONS ---
const showQRCode = async (recordId) => {
    try {
        ...
        qrLinkInput.value = data.emailLink;
        qrModal.classList.remove('hidden');
    } catch (error) {
        console.error('Error generating QR code:', error);
        alert('Could not generate QR code. Is the backend server running?');
    }
};
...
const downloadPDF = async (recordId) => {
    try {
        ...
        alert('Could not download PDF. Is the backend server running?');
    }
};
...
const viewRecord = (recordId) => {
    ...
    if (recordToView) {
        ...
        state.currentRecordId = recordId; // Set the current record ID for QR/PDF buttons
...
const handleSwitchMode = (mode) => {
    ...
    if(mode !== 'transcript') {
        state.analysis = null; // Clear analysis if leaving the page
        state.currentRecordId = null; // Clear the record ID
    }
    if (mode === 'records') {
        loadRecords(); // Fetch records when switching to the records view
    } else if (mode === 'statistics') {
        loadStatistics(); // Fetch statistics when switching to statistics view
        render();
    } else {
        render();
    }
...
const init = () => {
    ...
    statisticsModeBtn.addEventListener('click', () => handleSwitchMode('statistics'));
    ...
    analysisOutput.addEventListener('click', (e) => {
        ...
        if (e.target.closest('#qr-code-btn')) {
            if (state.currentRecordId) {
                showQRCode(state.currentRecordId);
            }
        }
        if (e.target.closest('#download-pdf-btn')) {
            if (state.currentRecordId) {
                downloadPDF(state.currentRecordId);
            }
        }
    });
    ...
    patientRecordsView.addEventListener('click', (e) => {
        ...
        const qrBtn = e.target.closest('.qr-record-btn');
        if (qrBtn) {
            showQRCode(qrBtn.dataset.recordId);
        }
        const pdfBtn = e.target.closest('.pdf-record-btn');
        if (pdfBtn) {
            downloadPDF(pdfBtn.dataset.recordId);
        }
    });
    ...
    copyQrLinkBtn.addEventListener('click', () => {
        ...
        navigator.clipboard.writeText(qrLinkInput.value).then(() => {
            ...
            setTimeout(() => {
                copyQrLinkBtn.textContent = '📋 Copy Link';
            }, 2000);
        });
    });
